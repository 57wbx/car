'use strict';
app.controller('carownerEditController', ['$rootScope','$scope', '$http', '$state',  'uiLoad', 'JQ_CONFIG','FileUploader','previewService','sessionStorageService','hintService',
  function($rootScope,$scope, $http, $state, uiLoad, JQ_CONFIG,FileUploader,previewService,sessionStorageService,hintService) {
	
	$scope.needCacheArray = ["carownerDataTableProperties","carownerIdForEdit"];
	sessionStorageService.clearNoCacheItem($scope.needCacheArray);
	if($scope.rowIds&&$scope.rowIds[0]){
		sessionStorageService.setItem("carownerIdForEdit",$scope.rowIds[0]);
	}else{
		$scope.rowIds[0]  = sessionStorageService.getItemStr("carownerIdForEdit");
	}
...
//						timePicker : true,
    ...
    $scope.submit = function(){
    	$http({
    		url:"base/memberAction!updateCarowner.action",
    		method:"post",
    		data:$scope.formData
    	}).then(function(resp){
    		if(resp.data.code==1){

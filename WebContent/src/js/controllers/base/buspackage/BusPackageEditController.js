app.controller("busPackageEditController",['$scope','$state','$http',function($scope,$state,$http){
	...
	$scope.submit = function(){
		...
		$http({
			url:"base/busPackageAction!saveBusPackage.action",
			method:'post',
			data : $scope.formData
		}).then(function(resp){
	...
	var setting = {
		...
		$http({
			...
			if(resp.data.code == 1){//返回成功
				...
				//初始化服务列表数据
				for(var i=0;i<resp.data.details.busItems.length;i++){
					showBusItemAndCacheBusItemId(resp.data.details.busItems[i]);
				}
				//初始化业务类型树的数据
				$scope.formData.busTypeName = resp.data.busTypeName;
				
				//将参数
				$scope.formData.busItems = undefined;

'use strict';
app.controller('carEditController', ['$scope', '$state','$http','hintService','sessionStorageService',
  function($scope, $state,$http,hintService,sessionStorageService) {
	
    $scope.formData = {};
    
    $scope.needCacheArray = ["carListDataTableProperties","carIdForEdit"];
	sessionStorageService.clearNoCacheItem($scope.needCacheArray);
	if($scope.rowIds&&$scope.rowIds[0]){
		sessionStorageService.setItem("carIdForEdit",$scope.rowIds[0]);
	}else{
		$scope.rowIds[0]  = sessionStorageService.getItemStr("carIdForEdit");
	}
